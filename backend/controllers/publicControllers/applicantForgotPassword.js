    ...
    try {
       ...
       const [existingApplicant] = await database.query(`
        ...
        await connection.query(
            ...
            `,
            [
                existingApplicant[0].applicantID,
                normalizedEmail,
                hashedCode,
                CODE_EXPIRY_MINUTES
            ]
...
export async function verifyPasswordCode(req, res) {
    ...
    try {
        const [[user]] = await database.query(
            `SELECT applicantID FROM applicants WHERE LOWER(email) = ? LIMIT 1`,
            [normalizedEmail]
        );
...
export async function resetPassword(req, res) {
    ...
    try {
        ...
        await connection.query(`
            DELETE FROM passwordResets WHERE userID = ? AND userType = 'applicant'`,
            [userID]
        );
