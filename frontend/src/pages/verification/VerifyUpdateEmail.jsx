...
export default function VerifyUpdateEmail({ user }) {
    ...
    async function handleSubmit(e) {
        ...
        try {
            let update;
            if (isApplicant) {
                update = await api.patch("/applicant/changeEmail/verify", { code });
            } else {
                update = await api.patch("/employer/changeEmail/verify", { code });
            }

            handleCurrentUser(update.data.user);
    ...
    async function resendCode() {
        ...
        try {
            if (isApplicant) {
                await api.post("/applicant/changeEmail/resendCode", { applicantEmail });
            } else {
                await api.post("/employer/changeEmail/resendCode", { employerEmail });
            }
    ...
    return (
        <main className="w-full min-h-screen bg-[#F3F4F6] relative p-6 md:p-15">
            ...
            {showPopUp && (
                <>
                    <Translucent />
                    <ConfirmationBox onClick={closePopUp} buttonText="Close" text="Email address updated successfully" />
                </>
            )}
            ...
            <div className="m-auto w-full md:w-100 lg:w-150">
                ...
                <p className="text-gray-800 font-medium mb-5">
                    To update your email address, enter the 6-digit code we sent to&nbsp;
